    ...
    private void initializeGrid() {
        ...
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                final int frow = row;
                final int fcol = col;
                square.setOnMouseClicked(event -> handleSquareClick(fcol, frow));
...
/*        grid[0][0] = new Rook(Color.WHITE, 0, 0);
    ...
    private void handleSquareClick(int col, int row) {
        System.out.println("Coordinate del quadrato cliccato: X = " + col + ", Y = " + row);

        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if(grid[i][j] != null && grid[i][j].isSelected())
                {
                    gridPane.getChildren().remove(grid[i][j].getImg());
                    grid[i][j].move(col, row);
                    gridPane.add(grid[i][j].getImg(), grid[i][j].getX(), grid[i][j].getY());
                }
            }
        }
